...
class _DBCtx(threading.local):
    '''
    Thead local object that holds connection info
    '''
...
def next_id(t=None):
    ...
    Args:
        t :unix timestamp,default oto None ans using time.time()
    '''
    if t is None:
        t = time.time()
...
def select_int(sql, *args):
    '''
    execute SQL and expect one int and only one int result
...
if __name__ == '__main__':
    ...
    update('create table user (id bigint primary key,name text,email text,passwd text,last_modified real)')
    import doctest
